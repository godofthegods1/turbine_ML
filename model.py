...
def objective(params):
    ...
    return -closest_row['volts']  # negative because minimize in gp_minimize


res = gp_minimize(objective, space, n_calls=70, acq_func='PI', random_state=42)

# train Gaussian Process Regression
X_train = df[['num_paddles', 'paddle_angle']].values
y_train = df['volts'].values
kernel = RBF(length_scale=10) + WhiteKernel(noise_level=0.2)
...
# plot 2D slices
plt.figure(figsize=(8, 5))
plt.scatter(df['num_paddles'], df['volts'], label="Real Data", color='blue')
...
plt.figure(figsize=(8, 5))
plt.scatter(df['paddle_angle'], df['volts'], label="Real Data", color='blue')
